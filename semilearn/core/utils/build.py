...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_json_dset, get_pkl_dset, get_tinyimagenet, get_adaptiope, get_cifar_os, get_cifar_os_outliers
    ...
    if dataset == "eurosat":
        ...
        test_dset = None
    elif dataset in ["cifaros10", "cifaros100"]:
        lb_dset, ulb_dset, eval_dset = get_cifar_os_outliers(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset == 'svhn':
        ...
        lb_dset, ulb_dset, eval_dset = get_tinyimagenet(args, algorithm, "tiny-imagenet-200", num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset == "adaptiope":
        lb_dset, ulb_dset, eval_dset = get_adaptiope(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    # speech dataset
